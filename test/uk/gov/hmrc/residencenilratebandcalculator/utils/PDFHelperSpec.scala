...
class PDFHelperSpec extends BaseSpec {
  ...
  private val cacheMapAllNonDecimalFields: CacheMap = new CacheMap(
    cacheMapKey,
    Map[String, JsValue](
  ...
  private def acroForm(
      ...
      generateWelshPDF: Boolean = false
  ): PDAcroForm = {
    ...
    val optionPDAcroForm: Option[PDAcroForm] =
      pdfHelper.generatePDF(filledCacheMap, generateWelshPDF = generateWelshPDF).map { baos =>
  ...
  private def checkMultipleFieldValues(
      acroForm: PDAcroForm,
      baseFieldName: String,
      expectedDate: String,
      totalFields: Int
  ): IndexedSeq[scalatest.Assertion] =
  ...
  private def describeTest(fieldName: String, isWelshTest: Boolean) = {
    val suffix = if (isWelshTest) { " in the welsh language" }
    else { "" }
  ...
  private def pdfFieldTestPart1(generateWelshPDF: Boolean): Unit = {
    ...
    describeTest("IHT435_10_1 to 7: decimal number of less than maximum size", isWelshTest = generateWelshPDF) in {
      val cacheMap: CacheMap = new CacheMap(
        cacheMapKey,
        Map[String, JsValue](
  ...
  private def pdfFieldTestPart3(generateWelshPDF: Boolean): Unit = {
    behave.like(pdfField(fieldName = "IHT435_21", expectedValue = "888", generateWelshPDF = generateWelshPDF))
    behave.like(
      pdfField(
        fieldName = "IHT435_22",
        expectedValue = yes(if (generateWelshPDF) Some("Oes") else None),
    ...
    behave.like(pdfField(fieldName = "IHT435_27", expectedValue = "3333", generateWelshPDF = generateWelshPDF))
    behave.like(pdfField(fieldName = "IHT435_28", expectedValue = "229988", generateWelshPDF = generateWelshPDF))
    ...
    describeTest("IHT435_26", isWelshTest = generateWelshPDF) + " when field in cache but it can be calculated" in {
      ...
      acroForm(filledCacheMap = cacheMap, generateWelshPDF = generateWelshPDF)
        .getField("IHT435_26")
