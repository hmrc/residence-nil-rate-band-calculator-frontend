...
class ValueOfAssetsPassingControllerSpec extends SimpleControllerSpecBase {
  ...
  "Value Of Assets Passing Controller" must {
    ...
    def createView = (value: Option[Map[String, String]]) => {
      value match {
        case None => value_of_assets_passing(frontendAppConfig, NonNegativeIntForm.apply(errorKeyBlank, errorKeyDecimal, errorKeyNonNumeric, errorKeyTooLarge),answerRows = Seq(), formattedPropertyValue = None)(fakeRequest, messages, applicationProvider)
        case Some(v) => value_of_assets_passing(frontendAppConfig, NonNegativeIntForm(errorKeyBlank, errorKeyDecimal, errorKeyNonNumeric, errorKeyTooLarge).bind(v), Seq(), None)(fakeRequest, messages, applicationProvider)
      }
    }

    def createController = () => new ValueOfAssetsPassingController(frontendAppConfig, messagesApi, mockSessionConnector, navigator, applicationProvider)
