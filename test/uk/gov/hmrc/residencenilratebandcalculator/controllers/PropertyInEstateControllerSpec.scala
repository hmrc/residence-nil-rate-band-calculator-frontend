...
class PropertyInEstateControllerSpec extends SimpleControllerSpecBase {
  ...
  "Property In Estate Controller" must {
    ...
    def createView = (value: Option[Map[String, String]]) => {
      value match {
        case None => property_in_estate(frontendAppConfig, BooleanForm.apply(messageKey), answerRows = Seq())(fakeRequest, messages, applicationProvider)
        case Some(v) => property_in_estate(frontendAppConfig, BooleanForm(messageKey).bind(v), Seq())(fakeRequest, messages, applicationProvider)
      }
    }

    def createController = () => new PropertyInEstateController(frontendAppConfig, messagesApi, mockSessionConnector, navigator, applicationProvider)
