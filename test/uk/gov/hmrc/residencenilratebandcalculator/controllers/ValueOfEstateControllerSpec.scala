...
class ValueOfEstateControllerSpec extends RnrbControllerSpec {
  ...
  "Value Of Estate Controller" must {
    ...
    def createController =
      () => new ValueOfEstateController(messagesControllerComponents, mockSessionConnector, navigator, value_of_estate)

    val testValue = 123

    behave.like(
      rnrbController[Int](createController, createView, Constants.valueOfEstateId, messageKeyPrefix, testValue)(
        Reads.IntReads,
        Writes.IntWrites
      )
    )
    ...
    behave.like(
      nonStartingController[Int](
        createController,
        List(Constants.dateOfDeathId, Constants.partOfEstatePassingToDirectDescendantsId)
