...
class ValueOfEstateControllerSpec extends SimpleControllerSpecBase {
  ...
  "Value Of Estate Controller" must {
    ...
    def createView = (value: Option[Map[String, String]]) => {
      ...
      value match {
        case None => value_of_estate(frontendAppConfig, NonNegativeIntForm.apply(errorKeyBlank, errorKeyDecimal, errorKeyNonNumeric, errorKeyTooLarge), answerRows = Seq())(fakeRequest, messages, applicationProvider)
        case Some(v) => value_of_estate(frontendAppConfig, NonNegativeIntForm(errorKeyBlank, errorKeyDecimal, errorKeyNonNumeric, errorKeyTooLarge).bind(v), Seq())(fakeRequest, messages, applicationProvider)
      }
    }
