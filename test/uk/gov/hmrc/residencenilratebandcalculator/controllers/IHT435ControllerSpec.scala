...
class IHT435ControllerSpec extends UnitSpec with WithFakeApplication with MockSessionConnector {
  ...
  private val filledcacheMap: CacheMap = new CacheMap("", Map[String, JsValue](
    "surname" -> JsString("Xyz"),
    "firstname" -> JsString("Jkl"),
    Constants.dateOfDeathId -> JsString("2017-5-12"),
    "refno" -> JsString("Pxmxzmx"),
    ...
    "addr1" -> JsString("addr1"),
    "addr2" -> JsString("addr2"),
    "addr3" -> JsString("addr3"),
    "postcode" -> JsString("XX11XX"),
    "valueofresidenceatdateofdeath" -> JsNumber(9948),
    "%ofrespassestodirect" -> JsString("88.8899"),
    "descendantname1" -> JsString("Xxx"),
    "descendantname2" -> JsString("Yyy"),
    "descendantname3" -> JsString("Zzz"),
    "rel1" -> JsString("aBc"),
    "rel2" -> JsString("aBd"),
    "rel3" -> JsString("aBe"),
    Constants.exemptionsAndReliefClaimedId -> JsBoolean(true),
    Constants.grossingUpOnEstatePropertyId -> JsBoolean(false),
    "chargeablevalueforiht" -> JsNumber(8893),
    "chargeablepart" -> JsNumber(8894),
    Constants.transferAnyUnusedThresholdId -> JsBoolean(true),
    "how much transferrable res nil rate" -> JsNumber(88728),
    Constants.claimDownsizingThresholdId -> JsBoolean(false),
    "addra1" -> JsString("addr1"),
    "addra2" -> JsString("addr2"),
    "addra3" -> JsString("addr3"),
    "postacode" -> JsString("XX11XX"),
    Constants.datePropertyWasChangedId -> JsString("2017-5-13"),
    "valueof deceased interest" -> JsNumber(888),
    Constants.partOfEstatePassingToDirectDescendantsId -> JsBoolean(true),
    Constants.grossingUpOnEstateAssetsId -> JsBoolean(false),
    "totalvalueofotherassets" -> JsNumber(777),
    "descendantnamea1" -> JsString("Xxx"),
    "descendantnamea2" -> JsString("Yyy"),
    "descendantnamea3" -> JsString("Zzz"),
    "rela1" -> JsString("aBc"),
    "rela2" -> JsString("aBd"),
    "rela3" -> JsString("aBe"),
    Constants.transferAvailableWhenPropertyChangedId -> JsBoolean(true),
    "amountoftnrb" -> JsNumber(8777),
