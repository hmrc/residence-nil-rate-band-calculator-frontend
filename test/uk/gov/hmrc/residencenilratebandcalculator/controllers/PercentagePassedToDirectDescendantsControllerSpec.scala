...
class PercentagePassedToDirectDescendantsControllerSpec extends SimpleControllerSpecBase {
  ...
  "Percentage Passed To Direct Descendants Controller" must {
    ...
    def createView = (value: Option[Map[String, String]]) => {
      value match {
        case None => percentage_passed_to_direct_descendants(frontendAppConfig, PositivePercentForm.apply(errorKeyBlank, errorKeyNonNumeric, errorKeyOutOfRange), answerRows = Seq())(fakeRequest, messages, applicationProvider)
        case Some(v) => percentage_passed_to_direct_descendants(frontendAppConfig,
          PositivePercentForm(errorKeyBlank, errorKeyNonNumeric, errorKeyOutOfRange).bind(v), Seq())(fakeRequest, messages, applicationProvider)
      }
    }

    def createController = () => new PercentagePassedToDirectDescendantsController(frontendAppConfig, messagesApi, mockSessionConnector, navigator, applicationProvider)
