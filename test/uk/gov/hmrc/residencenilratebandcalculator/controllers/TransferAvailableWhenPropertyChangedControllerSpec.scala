...
class TransferAvailableWhenPropertyChangedControllerSpec extends SimpleControllerSpecBase {
  ...
  "Transfer Available When Property Changed Controller" must {
    ...
    def createView = (value: Option[Map[String, String]]) => {
      value match {
        case None => transfer_available_when_property_changed(frontendAppConfig, BooleanForm.apply(messageKey),answerRows = Seq())(fakeRequest, messages, applicationProvider)
        case Some(v) => transfer_available_when_property_changed(frontendAppConfig, BooleanForm(messageKey).bind(v), Seq())(fakeRequest, messages, applicationProvider)
      }
    }

    def createController = () => new TransferAvailableWhenPropertyChangedController(frontendAppConfig, messagesApi, mockSessionConnector, navigator, applicationProvider)
