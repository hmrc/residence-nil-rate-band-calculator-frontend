...
class AnyExemptionViewSpec extends HtmlSpec {
  ...
  def fixture(form: Option[Form[Boolean]] = None) = new {
    val view = any_exemption(frontendAppConfig, url, form)(request, messages)
    val doc = asDocument(view)
  }

  "Any Exemption View" when {
    ...
    "rendered" must {
      ...
      "display the correct question designator" in {
        val f = thisFixture()
        assertContainsMessages(f.doc, "any_exemption.question_number")
      }

      "display the correct browser title" in {
        val f = thisFixture()
        assertEqualsMessage(f.doc, "title", "any_exemption.browser_title")
      }

      "display the correct page title" in {
        val f = thisFixture()
        assertPageTitleEqualsMessage(f.doc, "any_exemption.title")
      }

      "display the correct guidance" in {
        val f = thisFixture()
        assertContainsMessages(f.doc, "any_exemption.guidance")
      }
      ...
      "contain a form that POSTs to the correct action" in {
        ...
        form.attr("action") shouldBe AnyExemptionController.onSubmit().url
      }
      ...
      "contain a legend for the question" in {
        ...
        legends.first.text shouldBe messages("any_exemption.label")
      }
