...
class AnyDownsizingAllowanceViewSpec extends HtmlSpec {
  ...
  def fixture(form: Option[Form[Boolean]] = None) = new {
    val view = any_downsizing_allowance(frontendAppConfig, url, form)(request, messages)
    val doc = asDocument(view)
  }

  "Any Downsizing Allowance View" when {
    ...
    "rendered" must {
      ...
      "display the correct page title" in {
        val f = thisFixture()
        assertPageTitleEqualsMessage(f.doc, "any_downsizing_allowance.title")
      }

      "display the correct guidance" in {
        val f = thisFixture()
        assertContainsMessages(f.doc, "any_downsizing_allowance.guidance")
      }
      ...
      "contain a form that POSTs to the correct action" in {
        ...
        form.attr("action") shouldBe routes.AnyDownsizingAllowanceController.onSubmit().url
      }
      ...
      "contain a legend for the question" in {
        ...
        legends.first.text shouldBe messages("any_downsizing_allowance.label")
      }
