...
class ValueBeingTransferredViewSpec extends IntViewSpecBase {
  ...
  "Value Being Transferred View" must {
    ...
    behave like pageWithoutBackLink[Int](createView, Some(fakeApplication.injector.instanceOf[ValueBeingTransferredController].form()))

    behave like intPage(createView, messageKeyPrefix, ValueBeingTransferredController.onSubmit().url, NonNegativeIntForm(errorMessage, errorMessage, errorMessage), Some(fakeApplication.injector.instanceOf[ValueBeingTransferredController].form()))

    behave like pageContainingPreviousAnswers(createView, Some(fakeApplication.injector.instanceOf[ValueBeingTransferredController].form()))
