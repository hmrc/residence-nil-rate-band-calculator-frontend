...
class AssetsPassingToDirectDescendantsViewSpec extends HtmlSpec {
  ...
  def fixture(form: Option[Form[Int]] = None) = new {
    val view = assets_passing_to_direct_descendants(frontendAppConfig, url, form)(request, messages)
  ...
  "Assest Passing to Direct Descendants View" when {
    ...
    "rendered" must {
      ...
      "display the correct question designator" in {
        val f = thisFixture()
        assertContainsMessages(f.doc, "assets_passing_to_direct_descendants.question_number")
      }
