  ...
  "Any Assets Passing to Direct Descendants View" when {
    ...
    "rendered" must {
      ...
      "display the correct browser title" in {
        val f = thisFixture()
        assertEqualsMessage(f.doc, "title", "any_assets_passing_to_direct_descendants.browser_title")
      }

      "display the correct page title" in {
        val f = thisFixture()
        assertPageTitleEqualsMessage(f.doc, "any_assets_passing_to_direct_descendants.title")
      }

      "display the correct guidance" in {
        val f = thisFixture()
        assertContainsMessages(f.doc, "any_assets_passing_to_direct_descendants.guidance")
      }
      ...
      "contain a form that POSTs to the correct action" in {
        ...
        form.attr("action") shouldBe AnyAssetsPassingToDirectDescendantsController.onSubmit().url
      }
      ...
      "contain a legend for the question" in {
        ...
        legends.first.text shouldBe messages("any_assets_passing_to_direct_descendants.label")
      }
