...
class ThresholdCalculationResultViewSpec extends HtmlSpec {
  def fixture() = new {
    val view = threshold_calculation_result("£10.00", Nil)(request, messages, applicationProvider)
  ...
  "Threshold Calculation Result View" when {
    ...
    "rendered" must {
      ...
      "display the correct information when there is no Residence Nil Rate Amount" in {
        ...
        val view = threshold_calculation_result(residenceNilRateAmount, Nil)(request, messages, applicationProvider)
        val doc = asDocument(view)
