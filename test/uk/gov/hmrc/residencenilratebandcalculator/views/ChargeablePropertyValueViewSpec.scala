...
class ChargeablePropertyValueViewSpec extends IntViewSpecBase {
  ...
  "Chargeable Property Value View" must {
    ...
    behave like pageWithoutBackLink[Int](createView, Some(fakeApplication.injector.instanceOf[ChargeablePropertyValueController].form()))

    behave like intPage(createView, messageKeyPrefix, ChargeablePropertyValueController.onSubmit().url, NonNegativeIntForm(errorMessage, errorMessage, errorMessage), Some(fakeApplication.injector.instanceOf[ChargeablePropertyValueController].form()))

    behave like pageContainingPreviousAnswers(createView, Some(fakeApplication.injector.instanceOf[ChargeablePropertyValueController].form()))
