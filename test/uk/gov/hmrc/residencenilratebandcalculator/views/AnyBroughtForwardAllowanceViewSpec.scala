...
class AnyBroughtForwardAllowanceViewSpec extends HtmlSpec {
  ...
  def fixture(form: Option[Form[Boolean]] = None) = new {
    val view = any_brought_forward_allowance(frontendAppConfig, url, form)(request, messages)
  ...
  "Any Brought Forward Allowance View" when {
    ...
    "rendered" must {
      ...
      "display the correct page title" in {
        val f = thisFixture()
        assertPageTitleEqualsMessage(f.doc, "any_brought_forward_allowance.title")
      }

      "display the correct guidance" in {
        val f = thisFixture()
        assertContainsMessages(f.doc, "any_brought_forward_allowance.guidance")
      }
      ...
      "contain a form that POSTs to the correct action" in {
        ...
        form.attr("action") shouldBe AnyBroughtForwardAllowanceController.onSubmit().url
      }
      ...
      "contain a legend for the question" in {
        ...
        legends.first.text shouldBe messages("any_brought_forward_allowance.label")
      }
