...
class CannotClaimRNRBViewSpec extends HtmlSpec {
  ...
  "Cannot Claim RNRB View" must {
    "display the correct browser title" in {
      val doc = asDocument(cannot_claim_RNRB(frontendAppConfig, "", Call("", ""), Seq()))
    ...
    "display the correct guidance" in {
      val doc = asDocument(cannot_claim_RNRB(frontendAppConfig, s"$messageKeyPrefix.guidance ", Call("", ""), Seq()))
      assertContainsText(doc, messages(s"$messageKeyPrefix.guidance"))
    }

    "display the correct reason" in {
      val doc = asDocument(cannot_claim_RNRB(frontendAppConfig, s"$messageKeyPrefix.no_property_reason", Call("", ""), Seq()))
    ...
    "contain a link to the given page" in {
      ...
      val doc = asDocument(cannot_claim_RNRB(frontendAppConfig, "", Call("GET", exampleURL), Seq()))
